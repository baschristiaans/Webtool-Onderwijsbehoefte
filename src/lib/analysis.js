...
export function analyzeRichInterpretation({
  ...
  if (homeInput.pattern !== 'unknown') {
    interpretationSignals.push({
      id: 'home-pattern',
      prompt: homeInput.pattern === 'contrast' ? 'Thuissituatie contrasteert met het schoolbeeld.' : 'Thuissituatie bevestigt het schoolbeeld grotendeels.',
      strength: 1
    });
  }
  ...
  if (
    ...
    discrepancySignals.push(
      'Het profielbeeld en de toetsgegevens samen geven aanwijzingen voor mogelijke onderprestatie.'
    );
  }
  ...
  if (notes.trim()) {
    interpretationSummaryParts.push(`Notitie van de leerkracht: ${normalizeText(notes.trim())}.`);
  }
